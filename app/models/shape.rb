...
class Shape
  ...
  def sort_points
    ...
    @points = @points.sort_by { |p| [p.angle(lowest_yx_coordinate), -p.y] }
  ...
  def check_point(p = nil)
    ...
    if right_turn?(p1, p2, p3)
      ...
      # Recursivly check the second to last point in @hull_points to see if they still make a left turn
      check_point()
    end
  end
